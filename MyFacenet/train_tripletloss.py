...
def main(args):
    ...
    np.random.seed(seed=args.seed)#指定伪随机数的种子
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if args.pretrained_model:
                ...
                meta_file, ckpt_file = get_model_filenames(args.pretrained_model)
                saver = tf.train.import_meta_graph(os.path.join(args.pretrained_model, meta_file))
                saver.restore(sess, os.path.join(args.pretrained_model, ckpt_file))
...
def sample_people(dataset, people_per_batch, images_per_person):
    '''
        随机从指定人数的人中抽取指定数量的样本
        输入：
            dataset：由ImageClass构成的list
            people_per_batch:抽取的人数目
            images_per_person:每个人抽取的图片数目
        返回：
            image_paths:抽取的样本路径组成的list
            num_per_class:每个人实际抽取的图片数目(因为可能部分人的图片数<images_per_person>)
    '''
...
def parse_arguments(argv):
    ...
    parser.add_argument('--max_nrof_epochs', 
                        type=int,
    ...
    parser.add_argument('--image_size', 
                        ...
                        default=160)
    parser.add_argument('--people_per_batch', 
                        type=int,
                        help='Number of people per batch.', 
                        default=45)
    parser.add_argument('--images_per_person', 
                        type=int,
    ...
    parser.add_argument('--seed', 
                        ...
                        help='Random seed.', 
                        default=666)
    parser.add_argument('--learning_rate_schedule_file', 
                        type=str,
